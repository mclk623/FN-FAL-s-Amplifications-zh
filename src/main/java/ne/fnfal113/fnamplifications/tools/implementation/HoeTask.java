...
public class HoeTask {
    ...
    public void tillLand(Player player, Block clickedBlock) {
        if(!dirtBlocks.contains(clickedBlock.getType())) return;
        ...
        for(int a = areaMap.get("x"); a < areaMap.get("i"); a++) {
            for(int b = areaMap.get("z"); b < areaMap.get("j"); b++) {
                if(dirtBlocks.contains(clickedBlock.getRelative(a, 0, b).getType())) {
                    if (Slimefun.getProtectionManager().hasPermission(Bukkit.getOfflinePlayer(player.getUniqueId()), clickedBlock.getRelative(a, 0, b), Interaction.INTERACT_BLOCK)) {
    ...
    public void harvest(Player player, Block clickedBlock, ItemStack itemStack, boolean willReplant){
        ...
        for (int a = areaMap.get("x"); a < areaMap.get("i"); a++) {
            for (int b = areaMap.get("z"); b < areaMap.get("j"); b++) {
                if (Tag.CROPS.isTagged(clickedBlock.getRelative(a, 0, b).getType()) && clickedBlock.getRelative(a, 0, b).getBlockData().clone() instanceof Ageable) {
        ...
        if (k > 0) {
            Utils.sendMessage("已自动补种了 " + k + " 株作物", player);
        }
    }
